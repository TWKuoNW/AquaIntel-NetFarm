...
while(cap.isOpened()):
    ...
    for i in range(len(trajectories)):  # 繪製所有軌跡
        cv2.polylines(frame, [np.array(trajectories[i], dtype=np.int32)], isClosed=False, color=(100, 100, 255), thickness=2)
        cv2.arrowedLine(frame, trajectories[i][0], trajectories[i][-1], (0, 0, 255), 2, tipLength=0.1)  # 繪製軌跡
