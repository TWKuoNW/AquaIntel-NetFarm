...
sheet = workbook.active # 建立工作表
sheet.append(["count", "Value"]) # 建立兩個列元素

cube_size = 10 # 定義立方體的邊長和時間間隔
time_interval = 0.05  # 時間間隔
num_fish = 1000 # 定義魚的數量
mark_fish = 100 # 標記魚的數量
num_steps = 100000  # 模擬的步數
small_cube_size = 3.5 # 漁網大小
...
for _ in range(num_steps):
    ...
    fish_scatter._offsets3d = (fish_x, fish_y, fish_z) # 更新魚的位置
    fish_scatter._facecolor3d = fish_colors
    
    if(catch_count % 10 == 0):
        crit_x = random.uniform(0 , cube_size - small_cube_size ) 
        crit_y = random.uniform(0 , cube_size - small_cube_size) 
        crit_z = random.uniform(0 , cube_size - small_cube_size) 
        # 定義長方體的八個頂點
        vertices = np.array([
        ...
        yellow_fish_count = sum(1 for x, y, z , color in zip(fish_x, fish_y, fish_z, fish_colors) if 
                            ...
                            color == "yellow" )
        if(yellow_fish_count >= 3 ):
            # 改變alpha值確定計算到
            mesh.set_alpha(0.5)

            x = mark_fish * total_fish_count / yellow_fish_count
            check_calculator = check_calculator + 1
            save = save + x
            if(check_calculator != 0 ):
                print("預測魚數量" , x)
                print("預測" , check_calculator , "次平均:", (save / check_calculator))
                sheet.append([check_calculator ,  (save / check_calculator)])
