...
def stop():
    ...
    if(check):
        ...
        label_status.config(text="RUN",  bg = "#7FFF00")
        check = True
...
# 创建标签和按钮
label_status = tk.Label(root, text = "RUN", width=10, height=7)
label_status.grid(row=0, column=0)

button_stop = tk.Button(root, text = "RUN/STOP", command = stop, width=10, height=2)
